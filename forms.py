...
class User_register_form(Form):
    ...
    location_first = QuerySelectField(
        'location_first', query_factory=location_first, get_label='location', allow_blank=True)
...
class Team_register_form(Form):
    ...
    favorite_date = SelectField(u'선호요일', choices=[('월요일', '월요일'), ('화요일', '화요일'), (
        '수요일', '수요일'), ('목요일', '목요일'), ('금요일', '금요일'), ('토요일', '토요일'), ('일요일', '일요일')])
...
class Search_user_name_form(Form):
...
class Search_school_name_form(Form):
    schoolname = StringField(
        u'학교이름:',
        [validators.data_required(u'학교를 입력하시기 바랍니다.')],
        description={'placeholder': u'학교를 입력하세요.'}
    )

    submit = SubmitField("검색")


class Search_team_name_form(Form):
    teamname = StringField(
        u'팀이름:',
        [validators.data_required(u'팀이름을 입력하시기 바랍니다.')],
        description={'placeholder': u'팀이름을 입력하세요.'}
    )
